...
try:
    ...
    if __name__ == '__main__':
        ...
        regex       = r"data-px-common-categorymenu-click-name=\"Computer\s*&amp;\s*Elektronica:Computer\"(?P<value>[\s\S]*?)<li class=\"main-nav__item\">"
        CategoryBlock= ObjStringUtil.GetStringResult(response[0], regex)
        CategoryURLList = ObjStringUtil.GetArrayListWithRegex(CategoryBlock,r"<a href=\"(?P<value>[\s\S]*?)\"[\s\S]*?>")
                        ...
                        while Count <= numberOfPages and Count <= 3:
                            ...
                            for productURL in ProductURLs:
                                ...
                                if productURL is not None:
                                    ...
                                    # Get product info
                                    dicData = objRegularExpressionParser.GetParseData(product_page[0], productURL)
                                    
                                    # Save product
                                    if len(dicData) > 0:
                                        ObjDbOperations.SaveDictionaryIntoMySQLDB(dicData)
                                        print("\n%s scraped" % (dicData['ProductName']))
